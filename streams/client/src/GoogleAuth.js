  ...
  componentDidMount() {
    ...
    window.gapi.load('client:auth2', ()=> {
      window.gapi.client.init({
        ...
        // what will the oauth ask for
        scope: 'email'
      })
      .then(()=> {
        this.auth = window.gapi.auth2.getAuthInstance();
        this.onAuthChange(this.auth.isSignedIn.get());  // immediately update
        this.auth.isSignedIn.listen(this.onAuthChange); // and wait for changes
      })
    });
  }
